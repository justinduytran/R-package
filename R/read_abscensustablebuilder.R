...
#'
#' @return A data frame with 4 columns, the 'row', 'column', 'wafer' variables and a 'value' variable.
#' @export
...
read_abscensustablebuilder <- function(table,
  ...
  # Determine how many sheets (i.e. wafers) are present
  ...
  suppressMessages(for (i in sheets) {
    extract <- readxl::read_excel(
      ...
      range <-
        paste(
          xlcolconv(bound_top_left[1]),
          bound_top_left[2] + margin,
          ":",
          xlcolconv(bound_bottom_right[1]),
    ...
    if (isTRUE(wafers)) {
      wafer_name <- readxl::read_excel(
        ...
        range <- paste(
          xlcolconv(bound_top_left[1] - adjust_wafer),
